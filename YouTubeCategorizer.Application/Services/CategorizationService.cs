...
namespace YouTubeCategorizer.Application.Services
{
    ...
    public class CategorizationService : ICategorizationService
    {
        ...
        public CategorizationService(HttpClient httpClient, IConfiguration configuration)
        {
            ...
            // Custom ML model categorization
            _mlApiUrl = configuration["MLApi:Url"] ?? "http://localhost:5001";

            Console.WriteLine($"[Categorization Service] Using custom ML model at: {_mlApiUrl}");
        ...
        // ===== Google AI (Gemini) API implementation — preserved for reference =====
        ...
        // ===== Custom ML model implementation (active) =====
        private async Task<string> CategorizeAsync(string text)
        {
            // ML API expects { "text": "..." }
            var payload = new { text = text };
            var url = $"{_mlApiUrl.TrimEnd('/')}/categorize";
            ...
            {
                var errorContent = await response.Content.ReadAsStringAsync();
                Console.WriteLine($"[Categorization Service] ❌ ML API Error: {response.StatusCode}");
                Console.WriteLine($"[Categorization Service] Error Response: {errorContent}");
                throw new HttpRequestException($"ML API returned {response.StatusCode}: {errorContent}");
            }

            var responseString = await response.Content.ReadAsStringAsync();
            Console.WriteLine($"[Categorization Service] API Response: {responseString.Substring(0, Math.Min(200, responseString.Length))}");

            var result = JsonConvert.DeserializeObject<MLCategorizationResponse>(responseString);

            if (result == null || string.IsNullOrWhiteSpace(result.Category))
            {
                Console.WriteLine("[Categorization Service] ML API returned empty category");
                throw new InvalidOperationException("ML API returned empty category");
            }

            string category = result.Category.Trim();
            string confidenceText = result.Confidence.HasValue
                ? $" (confidence: {result.Confidence.Value:P1})"
                : string.Empty;

            Console.WriteLine($"[Categorization Service] → Category: {category}{confidenceText}");
            return category;
        }

        private sealed class MLCategorizationResponse
        {
